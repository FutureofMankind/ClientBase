...
def add_client(cur, first_name, last_name, email, phone=None):
    if first_name == None or last_name == None or email == None:
        print('Не указано Имя/Фамилия/Почта')
        return
    ...
    if phone is not None:
        cur.execute("""
            ...
            """, (new_client[0], phone))
        cur.fetchone()
    print(f'Клиент добавлен {new_client}')
...
if __name__ == '__main__':
    with psycopg2.connect(database="netology_clients_db", user="postgres", password="postgres") as conn:
        with conn.cursor() as cur:
            ...
            # add_client(cur, 'Карла', 'Харви', 'harvy@gmail.com', '+79828578739')
            # add_client(cur, 'Дэйв', 'Мастейн', 'guitargod@gmail.com')
            # add_client(cur, 'Дарья', None, None)
            # add_client(cur, 'Брюс', 'Дикинсон', None)
            #
            # all_clients(cur)
            #
            # print(get_phone(cur, '1', '+79828578739'))
            # add_phone(conn, cur, '2', '+79125678946')
            #
            # all_clients(cur)
            #
            # change_client(conn, cur, '1', 'Хайди}')
            #
            # all_clients(cur)
            #
            # delete_phone(cur, '1', '+79125678946')
            #
            # all_clients(cur)
            #
            # find_client(cur, 'Карла')
            # find_client(cur, None, 'Дикинсон')
            # find_client(cur, None, None, 'guitargod@gmail.com')
            #
            # delete_client(cur, 5)
            #
            all_clients(cur)